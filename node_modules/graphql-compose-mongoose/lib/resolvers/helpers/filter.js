...
function filterHelperArgs(typeComposer, model, opts) {
    ...
    if (opts.onlyIndexed) {
        const indexedFieldNames = utils_1.getIndexedFieldNamesForGraphQL(model);
    ...
    if (typeComposer.schemaComposer.hasInstance(filterTypeName, graphql_compose_1.InputTypeComposer)) {
        ...
        makeFieldsRecursiveNullable_1.makeFieldsRecursiveNullable(itc, { prefix, suffix });
        ...
        filterOperators_1.addFilterOperators(itc, model, opts);
    }
    return {
        filter: {
...
function filterHelper(resolveParams, aliases) {
    ...
    if (filter && typeof filter === 'object' && Object.keys(filter).length > 0) {
        ...
        filterOperators_1.processFilterOperators(filterFields);
    ...
    if (utils_1.isObject(resolveParams.rawQuery)) {
    ...
    Object.keys(filterFields).forEach((key) => {
        ...
        if (key.startsWith('$')) {
            clearedFilter[key] = Array.isArray(value)
                ? value.map((v) => utils_1.toMongoFilterDottedObject(v, aliases))
                : utils_1.toMongoFilterDottedObject(value, aliases);
        }
            ...
            else if (utils_1.isObject(alias)) {
                subAlias = alias;
            ...
            utils_1.toMongoFilterDottedObject(value, subAlias, clearedFilter, newKey);
        }
    });
